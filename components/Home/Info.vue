...
<template>
    <div
        v-if="!loading"
        @click="clickclose"
        class="fixed top-0 left-0 w-full h-full z-[60] bg-[#c4c4c4cc] flex justify-center items-center px-4 py-8"
    >
        <div
            id="temp"
            class="bg-main py-8 px-4 md:px-8 w-[850px] h-[90vh] rounded-xl flex flex-col items-center shadow-xl relative overflow-y-scroll noscrollbar"
        >
            ...
            <div class="w-full grid grid-row-2 md:grid-cols-2 gap-4">
                <div
                    class="row-span-1 md:col-span-1 bg-white flex flex-col px-4 py-2 text-main font-semibold space-y-1"
                >
                    <span>娛樂城品牌資訊:</span>
            ...
            <button
                class="bg-white text-main text-xl font-bold px-8 py-0.5 rounded-xl mt-5 transition ease-in-out hover:-translate-y-1 hover:scale-110 duration-300"
            >
                立即註冊
            </button>
        </div>
